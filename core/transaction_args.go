...
// live transaction.
func (args *TransactionArgs) ToMessage(globalGasCap uint64, baseFee *big.Int) (*Message, error) {
...
// Print shows TransactionArgs fields (jmlee)
func (args *TransactionArgs) Print() {
	fmt.Println("txArgs.From:", args.From)
	fmt.Println("txArgs.To:", args.To)
	gasInt, _ := strconv.ParseInt(args.Gas.String()[2:], 16, 64)
	fmt.Println("txArgs.Gas:", args.Gas, "/", gasInt)
	fmt.Println("txArgs.GasPrice:", args.GasPrice.ToInt())
	fmt.Println("txArgs.MaxFeePerGas:", args.MaxFeePerGas)
	fmt.Println("txArgs.MaxPriorityFeePerGas:", args.MaxPriorityFeePerGas)
	fmt.Println("txArgs.Value:", args.Value, "/", args.Value.ToInt())
	nonceInt, _ := strconv.ParseInt(args.Nonce.String()[2:], 16, 64)
	fmt.Println("txArgs.Nonce:", args.Nonce, "/", nonceInt)
	fmt.Println("txArgs.Input:", args.Input)

	if args.AccessList != nil {
		for _, accessInfo := range *args.AccessList {
			fmt.Println("  txArgs.AccessList:", accessInfo.Address.Hex())
			fmt.Println("    storageKeys:", accessInfo.StorageKeys)
		}
	}
